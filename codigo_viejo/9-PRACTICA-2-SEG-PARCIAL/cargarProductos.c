...
void cargarProductos(Producto array[], int tamanio) //**
{
    ...
    FILE *productoFILE = fopen("productos.csv", "r" ); //**
    ...
    {
        ref = fgets(buffer, 200, productoFILE); // Si sale mal devuelve NULL, Deja de leer cuando se encuentra un salto de linea //**
        refaux = strdup(ref);
        ...
        while (token != NULL)
        {
            ...
            if(i >= REGISTROS_SIN_PROCESAR)
            {
                ...
                {
                    producto.precioCosto = strtoul(cadena, NULL, 10); //**
                }
