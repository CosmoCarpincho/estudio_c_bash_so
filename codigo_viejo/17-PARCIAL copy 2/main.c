...
int main(void)
{
    ...
    int i7;
    for (i7 = 0; i7 < TAM_ARREGLO ; i7++)
    {
        movimientos[i7].idProducto = 0;
        movimientos[i7].cantidad = 0;
        movimientos[i7].codLugar = 0;
    }
    ...
    while (opcion != 'q')
    {
        ...
        switch (opcion)
        {
            ...
            case '4':
                ...
                for (i2 = 0; i2 < 10; i2++)
                {
                     printf("sucursal %d : dinero que se recaudo : %.2f\n",i2 , auditoria.sucursales[i2]);
                }
               ...
               if ( stocks[is].disponible < ((stocks[is].stock_minimo)* 0.5))
                {
                    ...
                    double dinero = (double)pedidos[ip].cantPedir * stocks[is].costo;
