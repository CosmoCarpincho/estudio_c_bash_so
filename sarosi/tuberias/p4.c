/* Como crear en C: ps -efl | cut -c5-12 | sort -u > archivo.txt
 * -elf da muchos datos
 * -c5-12 corta del caracter 5 al 12
 * -u unique. Quita los repetidos
 *  Ahora que redireccione la salida a un archivo
 * */
...
int main(int argc, char **argv) {
  ...
  if (pid > 0) {
    ...
    //    }
    close(STDOUT_FILENO);
    int fd_stdout = open("archivo.txt", O_CREAT | O_APPEND | O_WRONLY, 0755);
    printf("El fd es %d: \n",
           fd_stdout); // Como redireccione el stdin va a ir al archivo
